  ############
# 3 spine model fitting
# M.kinneen 01/08/2026t
############
library(ggplot2)
library(here)
library(dplyr)
...
threesp_pois <- glmmTMB( #Poisson model
  value ~ treatment + (1 | year_site) + (1 | day),
  data   = threesp_dat,
  family = poisson
)

threesp_n_binom <- glmmTMB( #Poisson model
  value ~ treatment + (1 | year_site) + (1 | day),
  data   = threesp_dat,
  family = nbinom2
)

#3.2 model seleciton and diags

#Compare the resiudals ob both models
sim_pois <- simulateResiduals(threesp_pois)
sim_nb   <- simulateResiduals(threesp_n_binom)
...
png(here("output", "figures",  "3-spine_nb_dharma.png"),
    width = 2400, height = 1200, res = 300)
plot(sim_nb)
dev.off()

#Resiudal rsults point towards nbinom, as poisson fails dharma diags.
#Poiss KS = < 0.0001, indicating sogncat deviation from poisson dist.
#Confrom with AIC
AIC(threesp_pois, threesp_n_binom) #compare AIC
#nbinom much better
...
# 3-spine model with negative binomial looks good based on resiudals
# and zero inflation doestn seem like an issue.

# 3.2 Model output and summary
summary(threesp_n_binom)


#Random effects
# Effect of day is approimately twice that of site. This indicates that that sampling
# conditions on the day had twice is much impact ton catch rates as the sie that was sampled.
# 
# 
...
emmeans::emmeans(threesp_n_binom, pairwise ~ treatment, type = "response")


# Amino catch rates are slighlty higher than control the same as the control (A = C*1.3)
# Eggs and egg juice yield catch rates that are 2.23 and 1.5 times higher
# than the control, respectively.
#When we consider pairwise comparisons, catch rates are 1.5 times higher than
...
emm_df <- as.data.frame(emmeans(threesp_n_binom, ~ treatment, type = "response"))
...
ggsave(here("output", "3-spine_catch_by_treatment.png"),
