...
sim_pois <- simulateResiduals(sc_n_binom)
sim_nb   <- simulateResiduals(sc_n_binom)

par(mfrow = c(2, 2))
plotQQunif(sim_pois)     # top-left:     Poisson QQ
plotResiduals(sim_pois)  # top-right:    Poisson residual vs. predicted
plotQQunif(sim_nb)       # bottom-left:  NB QQ
plotResiduals(sim_nb)    # bottom-right: NB residual vs. predicted
par(mfrow = c(1, 1))     # reset when you're done
...
testZeroInflation(simulateResiduals(sc_n_binom))
# p> 0.05, ratio = 0.93
